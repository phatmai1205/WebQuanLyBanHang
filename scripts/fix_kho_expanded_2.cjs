...
const regex = /spHtml \+= `<tr>\s*<td>\$\{sp\.MaSP\}<\/td><td><b>\$\{sp\.TenSP\}<\/b><\/td><td>\$\{sp\.TenLoaiSP\}<\/td>\s*<td>\$\{fm\(sp\.DonGia\)\}<\/td><td><b>\$\{sp\.TonKho\}<\/b><\/td><td>\$\{textAlert\}<\/td>\s*\$\{actionTd\}\s*<\/tr>`;/;

const replaceStr = `spHtml += \`<tr class="\${expandedClass}" style="cursor: pointer;" onclick="toggleInvExpand('\${sp.MaSP}')">
         ...
         if (isExpanded) {
             let imgUrl = sp.HinhAnh && sp.HinhAnh.startsWith('data:') ? sp.HinhAnh : 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100"><rect width="100" height="100" fill="%23ddd"/><text x="25" y="55" fill="%23555" font-family="sans-serif" font-size="16">IMG</text></svg>';
