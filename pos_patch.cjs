...
// 1. Add "Bán hàng" button to sidebar
const sidebarFooter = `<div class="sidebar-footer">`;
const posSidebarButton = `
        <div class="nav-item" onclick="switchModule('pos', this)" style="margin-bottom: 5px;">
          <span class="nav-icon">💵</span><span class="nav-text">Bán hàng</span>
        </div>
`;
html = html.replace(sidebarFooter, posSidebarButton + sidebarFooter);

// 2. Add module-pos section
const endOfStaffModule = `</section>\n\n        <!-- MODAL -->`;
...
.pos-icon-btn:hover {
    color: var(--accent);
        ...
        <section id="module-pos" class="section-module">
                ...
                <div class="pos-right">
                    <div class="pos-right-header">
                        <div class="pos-search-wrapper">
                            ...
                            <input type="text" id="pos-customer-search" placeholder="Nhập tên hoặc sđt tìm khách hàng..." oninput="posSearchCustomer()" autocomplete="off">
...
html = html.replace(endOfStaffModule, posStyles + endOfStaffModule);

// 3. Add POS Payment Modal + Filter Modal
const posModals = `
...
html = html.replace('<!-- Overlay Noti Sidebar -->', posModals + '\n  <!-- Overlay Noti Sidebar -->');

// 4. Inject JS logic for POS
const posScript = `
/* ---------------- POS LOGIC ---------------- */
let posCart = [];
let posSelectedCustomer = null; // null = Khách lẻ
...
function renderPosProducts() {
    ...
    items.forEach(p => {
        ...
        // In real app, we might use p.HinhAnh, for now use a placeholder
        ...
        itemEl.innerHTML = \`
            ...
            <div class="pos-cart-details">
                <div class="pos-cart-top">
                    <span style="flex:1;">\${item.MaSP} - \${item.TenSP}</span>
                </div>
...
function posSearchCustomer() {
    ...
    if(list.length > 0) {
        list.forEach(item => {
            ...
            el.onmousedown = function() { // onmousedown fires before onblur
...
function openPosFilterModal() {
    ...
    let catSet = new Set();
    appData.KHO.forEach(p => {
        if(p.NhomHang) catSet.add(p.NhomHang);
    });
    ...
    cats.forEach(c => {
        ...
        container.innerHTML += \`
            <label style="display:flex; gap:10px; align-items:center; cursor:pointer; padding:5px 0;">
...
function submitPosPayment() {
    ...
    // Re-render others in background
    calcAndDrawCharts();
    renderTables();
}

/** Hook POS init into page load */
const originalOnloadForPos = window.onload;
window.onload = function(e) {
    if(originalOnloadForPos) originalOnloadForPos(e);
    initPos();
};

`;

html = html.replace('// --- Init default dates & fetch data ---', posScript + '\n    // --- Init default dates & fetch data ---');
